// Dify REST API 客户端（共享库 webmcp-agent-chat-core，2026-09-16 协议配置扩展）。
//
...
//
// 边界红线：transport 由宿主注入（fetchImpl，对齐 a2a-client.ts 先例），零 chrome.*；
// 埋点只记 URL/事件/状态/耗时，绝不记录 Authorization 头与请求体（日志红线同 a2a-client）。
import {
...
export interface DifyRequestOptions {
  /** Dify api-key（Authorization: Bearer，宿主经 a2aTokens 注入，不落日志）。 */
...
/** 单次对话输入（消费方由 A2aAgentConfig 组装；inputs 为 Chatflow 默认值）。 */
export interface DifyChatInput {
  ...
  /** 终端用户标识（Dify 契约必填；宿主注入每安装稳定 uuid）。 */
  user: string;
...
/** 默认请求总超时（毫秒；对齐 a2a-tool-source 的阻塞 send 上限）。 */
...
/** Dify 缺省终端用户标识（宿主未注入稳定 uuid 时的兜底，仅保证请求可发）。 */
export const DIFY_FALLBACK_USER = 'webmcp-extension';

/** 把 AbortError 归一为确定性超时/终止文案（对齐 a2a-client 同名私有函数语义）。 */
function abortErrorMessage(error: unknown): string {
  const reason = error instanceof Error ? error.message : String(error);
  return reason.includes('超时') ? reason : '请求已被终止';
}

/** 校验 URL 是 HTTP(S)；统一 invalid-url 错误（与 A2A 端点同一安全底线）。 */
...
/** 组装超时 + 外部 signal 的联合 AbortController（对齐 a2a-client 同名私有函数语义）。 */
...
function parseBlockingPayload(payload: unknown): DifyChatResult {
  ...
  if (
    ...
    throw new A2aClientError('invalid-response', `Dify 业务错误：${record['message'] as string}`);
  }
  try {
...
export function createDifyClient(deps: DifyClientDeps): DifyClient {
  ...
  // 会把 this 绑到 deps 对象上抛 Illegal invocation（对齐 a2a-client.ts 注释）。
  ...
  return {
    async chat(input, options = {}) {
      ...
      try {
        ...
        if (!response.ok) {
          throw new A2aClientError('http', `Dify 接口返回 HTTP ${response.status}（chat-messages）`);
        }
        const contentType = response.headers.get('Content-Type') ?? '';
        if (contentType.includes('text/event-stream')) {
          return await consumeStreamResponse(response, { onLog, endpoint, idleTimeoutMs, startedAt });
        }
...
async function consumeStreamResponse(response: Response, context: StreamContext): Promise<DifyChatResult> {
  const { onLog, endpoint, idleTimeoutMs, startedAt } = context;
  const body = response.body;
  if (body === null) {
    throw new A2aClientError('invalid-response', 'event-stream 响应缺少可读流');
  }
  ...
  try {
    for (;;) {
      ...
      for (const payload of payloads) {
        ...
        if (event.event === 'error') {
          ...
          throw new A2aClientError('invalid-response', `Dify 流内错误：${detail}`);
        }
        // ping / agent_thought / message_file / tts_* / workflow_* 等 P0 忽略
      }
    }
    ...
    if (signalAbortedByIdle(error)) {
      throw new A2aClientError('network', `流空闲超时（${idleTimeoutMs}ms 内无新事件）`);
    }
    throw new A2aClientError('network', `流读取失败：${error instanceof Error ? error.message : String(error)}`);
  } finally {
    if (idleTimer !== undefined) clearTimeout(idleTimer);
  }
}

/** 判定错误是否为 reader.cancel(空闲置超时) 触发的终止。 */
