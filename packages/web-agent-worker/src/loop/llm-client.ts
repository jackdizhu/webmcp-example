// LLM HTTP 协议适配层（自包含移植自 webmcp-agent-chat-core/src/llm-client.ts，V0-2：两协议全量）。
// 仅依赖 fetch —— 适配 openai-compat 与 anthropic 两个协议，注入 fetchImpl
// 供 worker 线程（self.fetch 解构裸调用）或单元测试替换。
//
// 埋点红线：日志只记 URL/模型/数量/耗时/状态，绝不记录 Authorization 头与消息体。
// 日志落点经 onLog 注入（消费方接自己的日志设施），本库不反向依赖宿主模块；
// onLog 收到的 payload 不含鉴权信息，可原样落盘。
...
/** openai-compat 协议默认请求路径。 */
const DEFAULT_OPENAI_API_PATH = '/chat/completions';
/** anthropic 协议默认 apiPath 空串提示（与 API_PATH_EMPTY_HINT 对称）。 */
const ANTHROPIC_API_PATH_EMPTY_HINT = '请配置apiPath，如：/v1/messages';

/** LLM 服务配置（API Key 由消费方持有与持久化，禁止硬编码）。 */
 ...
 *
 * @param config 服务配置
 * @param fetchImpl fetch 实现，默认全局 fetch（worker/测试注入）
    ...
    async complete(messages, tools, signal) {
      ...
      try {
        response = await fetchImpl(url, {
          ...
          // 终止时中断请求（AbortError 由 agent-loop 统一转义为已终止语义）
          ...(signal ? { signal } : {}),
        });
      } catch (error) {
...
/**
 * 按协议类型创建 LLM 客户端（仅 openai-compat / anthropic 两个适配器）。
 * apiProtocol 缺省回退 openai-compat。
 */
