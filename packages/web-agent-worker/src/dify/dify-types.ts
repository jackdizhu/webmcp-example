// Dify REST API 领域类型与运行时校验（自包含移植自 webmcp-agent-chat-core/src/dify-types.ts）。
//
// 职责：Dify chat-messages 请求/响应的消费字段子集与网络数据校验。
// 协议基线：Dify Service API（POST /v1/chat-messages，Authorization: Bearer {api_key}）。
// 仅取本库实际消费的字段，未知字段不校验不透传。
//
// 边界红线：纯类型/纯函数，零 DOM、零宿主依赖；网络数据不可信，
// 所有进入领域逻辑的远端结构必须先经本模块校验。

/** blocking 模式的完整响应（application/json，消费子集）。 */
export interface DifyChatCompletionResponse {
  ...
  /** 元数据（usage / retriever_resources，不消费，仅透传日志计数）。 */
  metadata?: unknown;
}
...
/** 校验 blocking 响应：answer 必须为字符串；失败抛错（调用方包装为 invalid-response）。 */
